...
function endsWith(str: string, endStr: string) {
    ...
    const inStrEnd = str.substring(str.length - endStr.length, endStr.length);
...
function startsWith(str: string, startStr: string) {
    ...
    const inStrStart = str.substring(0, startStr.length);
...
class BBCodeParser {
    ...
    constructor(private bbTags: BBTag[], private options = { escapeHTML: false }) {
